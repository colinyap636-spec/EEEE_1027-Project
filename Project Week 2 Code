...
const float circumference_m = 3.14159 * wheel_diameter_m;
...
void hardturn() {
  if (l == 1) {
    ...
    while (digitalRead(IR1) == 1 && digitalRead(IR2) == 0) {}
  }
  else if (r == 1) {
    digitalWrite(IN1, HIGH);
    digitalWrite(IN2, LOW);
...
// =========================
void updateDistance(int rpm) {
  unsigned long now = millis();
  float dt = (now - lastTime) / 1000.0;
  lastTime = now;

  float rps = rpm / 60.0;
  distance_m += rps * circumference_m * dt;
}
...
void loop() {
  drive();   // ← restored as before

  unsigned long nowMicros = micros();

  if (nowMicros - lastPulseTime > 150000) {
    RPM = 0;
  }
  else if (T > 0) {
    RPM = 60000000UL / (T * 20);
